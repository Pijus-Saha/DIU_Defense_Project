...
# Ensure this list covers the medicines in your test folder!
MEDICINE_DB = [
...
def get_vocab():
    if not os.path.exists(TRAIN_DATA_DIR): 
        print(f"[Error] Train data not found: {TRAIN_DATA_DIR}")
        return 0, {}
...
def main():
    ...
    print(f"--- DETAILED ACCURACY BREAKDOWN ---")
    
    if not os.path.exists(TEST_DATA_DIR):
        print(f"Error: Folder not found: {TEST_DATA_DIR}")
        return
    ...
    # Counters
    count_pass = 0         # Both Correct
    count_ai_pass = 0      # AI Correct, System Incorrect
    count_sys_pass = 0     # AI Incorrect, System Correct (Saved!)
    count_fail = 0         # Both Incorrect

    print(f"\n{'FILENAME':<25} | {'RAW PRED':<15} | {'CORRECTED':<15} | {'RESULT'}")
    print("-" * 75)
        ...
        for f in files:
            ...
            try:
                ...
                if is_raw_correct and is_sys_correct:
                    ...
                    status = "AI-PASS" # AI was right, System broke it
                    count_ai_pass += 1
                elif not is_raw_correct and is_sys_correct:
                    status = "SYS-PASS" # System saved the day
                    count_sys_pass += 1
                else:
                    status = "FAIL"
                    count_fail += 1

                print(f"{real_label:<25} | {raw_pred:<15} | {corrected:<15} | {status}")
                
            except:
                print(f"{f:<25} | ERROR PROCESSING IMAGE")
