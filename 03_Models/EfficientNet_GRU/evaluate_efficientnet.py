...
class EfficientNet_GRU(nn.Module):
    def __init__(self, num_chars):
        ...
        # 1. Modify First Layer (Stride (2,1) for width preservation)
        self.effnet.features[0][0] = nn.Conv2d(1, 32, kernel_size=3, stride=(2, 1), padding=1, bias=False)
        
        # 2. STRIDE HACKING (Replicating training logic)
        ...
        # 3. Adapter Layer (FIXED SIZE to 2560)
        # 1280 channels * 2 height = 2560
...
def main():
    ...
    print("--- EFFICIENTNET-GRU EVALUATION ---")
    ...
    model = EfficientNet_GRU(num_chars).to(device)
    ...
    # --- FINAL REPORT ---
    print("\n" + "="*50)
    print("      EFFICIENTNET PERFORMANCE REPORT")
    print("="*50)
